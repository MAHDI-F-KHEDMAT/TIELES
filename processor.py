...
# تغییر یافته به "KHANEVADEH_TLS_base64.txt"
...
# الگوی Regex برای شناسایی و پارس کردن کانفیگ‌های VLESS که فقط از security=tls استفاده می‌کنند.
VLESS_TLS_REGEX = re.compile(
    r"vless:\/\/"
    ...
    r"\?security=(?P<security>tls)"  # Security: فقط TLS پذیرفته می‌شود.
    r"(?:&flow=(?P<flow>[^&]+))?"  # Optional Flow
    r"(?:&fp=(?P<fingerprint>[^&]+))?"  # Optional Fingerprint
    r"(?:&pbk=(?P<publicKey>[^&]+))?"  # Optional PublicKey (برای Reality است، اما چون فقط TLS را هدف قرار می‌دهیم، این بخش نادیده گرفته می‌شود)
    r"(?:&sni=(?P<sni>[^&]+))?"  # Optional SNI (Server Name Indication)
    r"(?:&sid=(?P<sessionId>[^&]+))?"  # Optional Session ID (برای Reality است، نادیده گرفته می‌شود)
...
def parse_vless_config(vless_link: str) -> Optional[Dict[str, Union[str, int]]]:
    ...
    این تابع به طور خاص از VLESS_TLS_REGEX استفاده می‌کند که فقط کانفیگ‌های TLS را می‌پذیرد.
    """
    match = VLESS_TLS_REGEX.match(vless_link)
    if not match:
        return None # اگر با الگوی Regex مطابقت نداشت (یعنی TLS نبود یا فرمت اشتباه بود)، None برمی‌گرداند.
...
def process_subscription_content(content: str, source_url: str) -> List[Dict[str, Union[str, int]]]:
    ...
    for line in content.splitlines():
        line = line.strip()
        # فیلتر کردن اولیه: خط باید شامل SECURITY_KEYWORD ("tls") باشد، با "vless://" شروع شود و طول کافی داشته باشد.
        if SECURITY_KEYWORD in line and line.startswith("vless://") and len(line) > 50:
            parsed_data = parse_vless_config(line) # تلاش برای پارس کردن لینک با Regex مخصوص TLS

            # اگر پارس با موفقیت انجام شد و security آن دقیقاً "tls" بود:
